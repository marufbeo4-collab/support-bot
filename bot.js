...
const MAIN_GROUP_ID = -5184100145; // Apnar Admin Group ID
const ADMIN_IDS = [123456789]; // Apnar ID
...
console.log("🚀 GOWIN Support Bot Started with Mutex Lock!");
...
const LOCK_FILE = "polling.lock"; // 🔥 Notun Ghost Killer File
...
// ==============================
// 🔥 ATOMIC LOAD / SAVE (File Corruption Protector)
// ==============================
...
function saveJson(file, data) {
  try {
    // 🔥 Atomic Save: Age temp file e likhbe, tarpor asol file replace korbe. (Prevent corruption)
    const tmp = file + ".tmp";
...
function loadBlockedUsers() {
  try {
    ...
    const content = fs.readFileSync(BLOCK_FILE, "utf8");
...
function persistBlockedUsers() {
  try {
    ...
    fs.writeFileSync(tmp, JSON.stringify([...blockedUsers], null, 2));
...
// ==============================
// MUTEX LOCK (GHOST PROCESS KILLER)
// ==============================
function acquirePollLock() {
  try {
    const now = Date.now();
    if (fs.existsSync(LOCK_FILE)) {
      const lockTime = Number(fs.readFileSync(LOCK_FILE, "utf8") || 0);
      if (now - lockTime < 10000) {
        return false; // Onno ekta ghost process already polling kortese, tai ata ghumabe.
      }
    }
    fs.writeFileSync(LOCK_FILE, now.toString());
    return true;
  } catch {
    return true;
  }
...
function clearUserMessageCache(userId) {
  ...
  // 🔥 Delete every single cache related to this user (spam, warn, etc)
  for (const key of Object.keys(diskCache)) {
    if (key.includes(uid)) delete diskCache[key];
  }
  for (const key of [...runtimeProcessedMessages]) {
    if (key.includes(uid)) runtimeProcessedMessages.delete(key);
  }
...
function unblockUser(userId) {
  ...
  clearUserMessageCache(userId); // Etai ashol kaj korbe unblock er por msg ashar jonno
  return persistBlockedUsers();
}
...
function getUserName(msg) { return `${msg.from?.first_name || "User"}${msg.from?.username ? ` (@${msg.from.username})` : ""}`; }
function getUserLink(msg) { return `<a href="tg://user?id=${msg.chat.id}">${escapeHtml(getUserName(msg))}</a>`; }
...
function setCategory(userId, category) { userProfile[userId] = userProfile[userId] || {}; userProfile[userId].category = category; saveProfiles(); }
function getCategory(userId) { return userProfile[userId]?.category || "GENERAL"; }
function setLastTicket(userId, ticketId) { userProfile[userId] = userProfile[userId] || {}; userProfile[userId].lastTicketId = ticketId; saveProfiles(); }
function getLastTicket(userId) { return userProfile[userId]?.lastTicketId || null; }
function setLastTicketStatus(userId, status) { userProfile[userId] = userProfile[userId] || {}; userProfile[userId].lastTicketStatus = status; saveProfiles(); }
...
const mainKeyboard = { keyboard: [[{ text: BTN_DEPOSIT }, { text: BTN_WITHDRAW }], [{ text: BTN_LOGIN }, { text: BTN_OTHER }], [{ text: BTN_STATUS }, { text: BTN_HELP }]], resize_keyboard: true };
...
async function sendAlbumGroup(groupId) {
  ...
  await api("sendMessage", { chat_id: MAIN_GROUP_ID, text: buildCompactTicketMessage(firstMsg, category, ticketId, msgWithCaption?.caption || "Album Media"), parse_mode: "HTML", reply_markup: getAdminInlineKeyboard(userId) });
...
async function handlePrivateMessage(msg) {
  ...
  if (isBlocked(userId)) {
    ...
    const now = Date.now();
    if (now - (diskCache[`warn_${userId}`] || 0) > 300000) {
      await api("sendMessage", { chat_id: userId, text: `🚫 <b>ACCOUNT SUSPENDED</b>\n\nYou have been blocked by the admin.`, parse_mode: "HTML", reply_markup: { remove_keyboard: true } });
  ...
  if (tCheck.includes("deposit") || text === BTN_DEPOSIT) { setCategory(userId, "DEPOSIT"); await api("sendMessage", { chat_id: userId, text: categoryPrompt("DEPOSIT"), parse_mode: "HTML" }); return; }
  if (tCheck.includes("withdraw") || text === BTN_WITHDRAW) { setCategory(userId, "WITHDRAW"); await api("sendMessage", { chat_id: userId, text: categoryPrompt("WITHDRAW"), parse_mode: "HTML" }); return; }
  if (tCheck.includes("login") || tCheck.includes("game id") || text === BTN_LOGIN) { setCategory(userId, "LOGIN"); await api("sendMessage", { chat_id: userId, text: categoryPrompt("LOGIN"), parse_mode: "HTML" }); return; }
  if (tCheck.includes("other") || tCheck.includes("support") || text === BTN_OTHER) { setCategory(userId, "OTHER"); await api("sendMessage", { chat_id: userId, text: categoryPrompt("OTHER"), parse_mode: "HTML" }); return; }
  if (tCheck.includes("how to submit") || text === BTN_HELP) { await api("sendMessage", { chat_id: userId, text: helpText(), parse_mode: "HTML" }); return; }
  if (tCheck.includes("ticket status") || text === BTN_STATUS) {
    const ticketId = getLastTicket(userId);
    if (!ticketId) { await api("sendMessage", { chat_id: userId, text: `📌 <b>No recent ticket found.</b>\n\nPlease select an issue type and send your message first.`, parse_mode: "HTML" }); return; }
  ...
  if (msg.media_group_id) {
    if (!albumBucket[msg.media_group_id]) albumBucket[msg.media_group_id] = { firstMsg: msg, messages: [], timer: setTimeout(() => sendAlbumGroup(msg.media_group_id), 1800) };
    albumBucket[msg.media_group_id].messages.push(msg);
    return;
  }
  ...
  const diskCache = loadJson(MSG_CACHE_FILE, {});
  const now = Date.now();
  const isSpam = (now - (diskCache[`spam_${userId}`] || 0)) < 4000;
  ...
  if (text && !msg.photo && !msg.video && !msg.voice && !msg.document && !msg.sticker) {
    await api("sendMessage", { chat_id: MAIN_GROUP_ID, text: buildCompactTicketMessage(msg, category, ticketId, text), parse_mode: "HTML", disable_web_page_preview: true, reply_markup: getAdminInlineKeyboard(userId) });
    if (!isSpam) await api("sendMessage", { chat_id: userId, text: `✅ <b>Support request received</b>\n\nTicket: <code>${ticketId}</code>\nCategory: <b>${escapeHtml(category)}</b>\nStatus: <b>OPEN</b>`, parse_mode: "HTML" });
    return;
  }

  await api("copyMessage", { chat_id: MAIN_GROUP_ID, from_chat_id: userId, message_id: msg.message_id });
  await api("sendMessage", { chat_id: MAIN_GROUP_ID, text: buildCompactTicketMessage(msg, category, ticketId, msg.caption || "Media / File"), parse_mode: "HTML", reply_markup: getAdminInlineKeyboard(userId) });
  if (!isSpam) await api("sendMessage", { chat_id: userId, text: `✅ <b>Support request received</b>\n\nTicket: <code>${ticketId}</code>\nCategory: <b>${escapeHtml(category)}</b>\nStatus: <b>OPEN</b>`, parse_mode: "HTML" });
}
...
async function handleCallbackQuery(cb) {
  ...
  if (adminGroupId !== MAIN_GROUP_ID) return api("answerCallbackQuery", { callback_query_id: cb.id });
  if (!(await isGroupAdmin(adminGroupId, cb.from.id))) return api("answerCallbackQuery", { callback_query_id: cb.id, text: "⛔ Action Denied: Admins only!", show_alert: true });
  ...
  if (action === "close") {
    ...
    await api("editMessageReplyMarkup", { chat_id: adminGroupId, message_id: cb.message.message_id, reply_markup: { inline_keyboard: [[{ text: "🚫 Block User", callback_data: `block_${targetUserId}` }, { text: "✅ Unblock", callback_data: `unblock_${targetUserId}` }]] } });
...
async function handleGroupMessage(msg) {
  if (msg.text === "/id") return api("sendMessage", { chat_id: msg.chat.id, text: `🆔 <b>Group ID:</b> <code>${msg.chat.id}</code>`, parse_mode: "HTML" });
  ...
  if (isBlocked(meta.userId)) return api("sendMessage", { chat_id: msg.chat.id, text: `⚠️ <b>Action Failed:</b> This user is currently blocked. Click 'Unblock' on their ticket first.`, parse_mode: "HTML" });
  if (!meta.userMsgId) return;

  const replyKey = `reply_${msg.chat.id}_${msg.message_id}`;
...
// ==============================
// POLLING (WITH GHOST PROCESS LOCK)
// ==============================
async function poll() {
  ...
  while (true) {
    try {
      if (!acquirePollLock()) {
        await sleep(5000); // Wait! Onno ekta ghost process already kaj korche.
        continue;
      }
